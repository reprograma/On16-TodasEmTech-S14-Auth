...
const createPokemon = async (req, res) => {
   try {
     ...
     if(!authHeader) {
      return res.status(401).send('Você precisa de authorização.')
     }
...
const findAllPokemons = async (req, res) => {
   try {
    ...
    await jwt.verify(token, SECRET,async function(erro) {
      if(erro) {
        return res.status(403).send('Sem a autorização, não vai rolar.')
      }
...
const findPokemonById = async(req, res) => {
  try {
    const authHeader = req.get('authorization')
    if(!authHeader) {
      return res.status(401).send('Cadê a autorização?')
    }
