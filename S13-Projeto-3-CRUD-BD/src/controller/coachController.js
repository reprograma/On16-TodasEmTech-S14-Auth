 ...
 */
 ...
 const createCoach = async (req, res) => {
   try {
     ...
     await jwt.verify(token, SECRET, async function (erro) {
       ...
       if (erro) {
         return res.status(403).send('Insira um token válido.')
       }
 ...
 const findAllCoaches = async (req, res) => {
   try {
     ...
     await jwt.verify(token, SECRET, async function (erro) {
       ...
       if (erro) {
         return res.status(403).send('Token não autorizado! Informe um novo token.')
       }
 ...
 const findCoachById = async (req, res) => {
   try {
     ...
     await jwt.verify(token, SECRET, async function (erro) {
       if (erro) {
         return res.status(403).send('Informe um token válido.')
       }
